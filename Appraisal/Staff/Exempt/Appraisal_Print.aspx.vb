...
Public Class Appraisal_Print1
    ...
    Protected Sub SetLevel_Approval()
        ...
        SQL1 = "select Top 1 A.emplid,(select first+' '+last from id_tbl where emplid=a.emplid)Employee, A.MGT_EMPLID,B.MGT_EMPLID,(select first+' '+last from id_tbl where "
        SQL1 &= " emplid=Rtrim(Ltrim(A.MGT_EMPLID)))Current_MGT,(select first+' '+last from id_tbl where emplid=Rtrim(Ltrim(B.MGT_EMPLID)))Collab_MGT from Appraisal_MasterHistory_tbl A "
        SQL1 &= " JOIN Appraisal_Master_tbl B ON A.EMPLID=B.EMPLID where Rtrim(Ltrim(A.emplid)) in (" & lblEMPLID.Text & ") and A.Perf_Year=B.Perf_Year and A.Perf_Year=" & lblYEAR.Text & " "
        SQL1 &= " and LOGIN_EMPLID>0 order by DateTime desc"
