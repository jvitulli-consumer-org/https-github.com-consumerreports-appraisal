...
Public Class Appraisal_Edit_Guild
    ...
    Protected Sub DisplayData()
        ...
        ElseIf DT.Rows(0)("Process_flag").ToString = 1 Then
            ...
            txbOverall_Summary.Text = Replace(Replace(Replace(Replace(Replace(DT.Rows(0)("Overall_Summary").ToString, Chr(13), "<br>"), "{", "<"), "}", ">"), "`", "'"), "&nbsp;", " ")
            lblOverall_Summary.Text = Replace(Replace(Replace(Replace(DT.Rows(0)("Overall_Summary").ToString, Chr(13), "<br>"), "{", "&lt;"), "}", "&gt;"), "`", "'")
    ...
    Protected Sub Discussion_Comments()
        ...
        SqlDataSource2.SelectCommand &= " (select IsNull(Max(DateTime),'01/01/'+convert(char(4),Year(Getdate()))) from Appraisal_Discussion_tbl A where Perf_Year=" & lblYEAR.Text & " and emplid=" & lblEMPLID.Text & " and Comments='Edit') "
    ...
    Protected Sub SendToGeneralist()
        ...
        Msg &= "Email to Generalis " & lblHR_Email.Text

        '--Production email--
        'LocalClass.SendMail(lblHR_Email.Text, "Performance Appraisal was sent to you for Approval by " & Session("NAME"), Msg)

        'LocalClass.SendMail("vituja@consumer.org", "Performance Appraisal was sent to you for Approval by " & Session("NAME"), Msg)

        Response.Redirect("Appraisal_Edit_Guild.aspx?Token=" & z)
    ...
    Protected Sub Accomplishments_TextChanged(sender As Object, e As EventArgs) Handles txbAccomplishment.TextChanged
        SQL &= " Update Appraisal_Accomplishments_TBL Set Accomplishment= '" & Replace(Replace(Replace(Replace(txbAccomplishment.Text, "'", "`"), "<", "{"), ">", "}"), " ", "&nbsp;") & "' "
    ...
    Protected Sub Strengths_TextChanged(sender As Object, e As EventArgs) Handles txbStrengths.TextChanged
        SQL &= " Update Appraisal_Master_TBL Set Strengths= '" & Replace(Replace(Replace(Replace(txbStrengths.Text, "'", "`"), "<", "{"), ">", "}"), " ", "&nbsp;") & "' "
        SQL &= " where IndexId=1 and Perf_Year = " & lblYEAR.Text & " and EMPLID = " & lblEMPLID.Text & " "
        DT1 = LocalClass.ExecuteSQLDataSet(SQL1)
        LocalClass.CloseSQLServerConnection()
        ResetYellowColor()
    ...
    Protected Sub Development_TextChanged(sender As Object, e As EventArgs) Handles txbDevelopment.TextChanged
        SQL &= " Update Appraisal_Master_TBL Set Development_Area= '" & Replace(Replace(Replace(Replace(txbDevelopment.Text, "'", "`"), "<", "{"), ">", "}"), " ", "&nbsp;") & "' "
        SQL &= " where IndexId=1 and Perf_Year = " & lblYEAR.Text & " and EMPLID = " & lblEMPLID.Text & " "
        DT1 = LocalClass.ExecuteSQLDataSet(SQL1)
        LocalClass.CloseSQLServerConnection()
        ResetYellowColor()
    ...
    Protected Sub Overall_Sum_TextChanged(sender As Object, e As EventArgs) Handles txbOverall_Summary.TextChanged
        SQL &= " Update Appraisal_Master_TBL Set Overall_Summary= '" & Replace(Replace(Replace(Replace(txbOverall_Summary.Text, "'", "`"), "<", "{"), ">", "}"), " ", "&nbsp;") & "' "
        SQL &= " where IndexId=1 and Perf_Year = " & lblYEAR.Text & " and EMPLID = " & lblEMPLID.Text & " "
        DT1 = LocalClass.ExecuteSQLDataSet(SQL1)
        LocalClass.CloseSQLServerConnection()
        ResetYellowColor()
    ...
    Protected Sub btnSendEmployee_Click(sender As Object, e As EventArgs) Handles btnSendEmployee.Click
        ...
        Msg &= "Please click on the link http://" & Request.Url.Host & "/Appraisal/Default.aspx for full details.<br>"
        Msg &= "Email to employee " & lblEmpl_Email.Text

        '--Production email 
        'LocalClass.SendMail(lblEmpl_Email.Text, "Your Performance Appraisal is ready for your review", Msg)

        'LocalClass.SendMail("vituja@consumer.org", "Your Performance Appraisal is ready for your review", Msg)
