...
Partial Public Class GuildReview
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        'Response.Write(Session("EMPLID_LOGON") & "<br>" & Session("YEAR")) : Response.End()


        SQL4 = "select Process_Flag FLAG from GUILD_Appraisal_Master_tbl where emplid=" & Session("EMPLID_LOGON") & " and Perf_Year=" & Session("YEAR")
        ...
        If DT4.Rows.Count > 0 Then
            ...
            If CDbl(DT4.Rows(0)("FLAG").ToString) = 5 Then
                ...
                End If
                '--Guild information---
                SQL1 = "select (case when New_Employee=1 then 'SHORT' else 'FULL' end)Elig_Review,* from(Select Datediff(dd, convert(datetime,Hire_date),"
                SQL1 &= "convert(datetime,'09/30/'+convert(char(4),Perf_Year)))Elig_Perf,Date_Sent Mgr_Apr,Date_Submitted_To_HR UP_Mgt_Apr,Date_HR_Approved HR_Apr,Date_Guild_Reviewed Guild_Review,Year(IsNull(Refuse_date,0))Refuse_date,"
                SQL1 &= "Guild_Comments, a.emplid,First+' '+Last Guild_Name,email,jobtitle1,Departname,convert(char(10),Hire_date,101)Hired,Left(convert(decimal,datediff(day,SERVICE_DATE, GetDate()))/365.25,5)"
                SQL1 &= "YearsCU, " & Session("YEAR") & " CY,SUP_EMPLID,(select First+' '+last from id_tbl where emplid=SUP_EMPLID)SUP_NAME,(select email from id_tbl where emplid=SUP_EMPLID)SUP_EMAIL,UP_MGT_EMPLID,"
                SQL1 &= "(select First+' '+last from id_tbl where emplid=UP_MGT_EMPLID)UP_MGT_NAME,(select email from id_tbl where emplid=UP_MGT_EMPLID)UP_MGT_Email,HR_EMPLID,(select First+' '+last from id_tbl "
                SQL1 &= "where emplid=HR_EMPLID)HR_NAME,(select email from id_tbl where emplid=HR_EMPLID)HR_email,Len(IsNull(HR_EMPLID,''))SEND_HR,Process_Flag,New_Employee from id_tbl a,Guild_Appraisal_MASTER_tbl b "
                SQL1 &= "where a.emplid=b.emplid and a.emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ")A"
                ...
                Session("HR_Apr") = DT1.Rows(0)("HR_Apr").ToString
            ...
            Else
                ClientScript.RegisterClientScriptBlock(Me.GetType, "Login", "<script language='JavaScript'> alert('No Appraisal is waiting for Review.'); window.location.href='http://crnet/'; </script>")
            End If
        Else
            ClientScript.RegisterClientScriptBlock(Me.GetType, "Login", "<script language='JavaScript'> alert('No Appraisal is waiting for Review.'); window.location.href='http://crnet/'; </script>")
        End If
    ...
    Protected Sub GridViewDisplay()
        ...
        SQL &= " from Guild_Appraisal_FACTORS_tbl where Perf_year=" & Session("YEAR") & " and emplid=" & Session("EMPLID_LOGON")
        ...
        If DT.Rows(0)("Index1").ToString = 1 Then
            ...
            SqlDataSource1.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=1 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""

        End If

        If CDbl(DT.Rows(0)("Index2").ToString) = 1 Then
            Panel2.Visible = True : GridView2.GridLines = GridLines.None
            SqlDataSource3.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=2 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div2.Visible = True
        End If

        If CDbl(DT.Rows(0)("Index3").ToString) = 1 Then
            Panel3.Visible = True : GridView3.GridLines = GridLines.None
            SqlDataSource3.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=3 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div3.Visible = True
        End If

        If DT.Rows(0)("Index4").ToString = 1 Then
            Panel4.Visible = True : GridView4.GridLines = GridLines.None
            SqlDataSource4.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=4 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div4.Visible = True
        End If

        If DT.Rows(0)("Index5").ToString = 1 Then
            Panel5.Visible = True : GridView5.GridLines = GridLines.None
            SqlDataSource5.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=5 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div5.Visible = True
        End If

        If DT.Rows(0)("Index6").ToString = 1 Then
            Panel6.Visible = True : GridView6.GridLines = GridLines.None
            SqlDataSource6.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=6 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div6.Visible = True
        End If

        If DT.Rows(0)("Index7").ToString = 1 Then
            Panel7.Visible = True : GridView7.GridLines = GridLines.None
            SqlDataSource7.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=7 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div7.Visible = True
        End If

        If DT.Rows(0)("Index8").ToString = 1 Then
            Panel8.Visible = True : GridView8.GridLines = GridLines.None
            SqlDataSource8.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=8 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div8.Visible = True
        End If
        If DT.Rows(0)("Index9").ToString = 1 Then
            Panel9.Visible = True : GridView9.GridLines = GridLines.None
            SqlDataSource9.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=9 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div9.Visible = True
        End If

        If DT.Rows(0)("Index10").ToString = 1 Then
            Panel10.Visible = True : GridView10.GridLines = GridLines.None
            SqlDataSource10.SelectCommand = "select * from Guild_Appraisal_FACTORS_tbl where IndexID=10 and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
            Div10.Visible = True
        End If
    End Sub
    ...
    Protected Sub DisplayData()
        ...
        SQL5 &= " from Guild_Appraisal_FACTORSUMMARY_tbl where emplid=" & Session("EMPLID_LOGON") & " and Perf_Year=" & Session("YEAR") & " )A group by emplid,Perf_Year)A,(select IsNull(MGR_Comments,'')MGR_Comments,"
        ...
        SQL5 &= "  'Unsatisfactory' else '' end)Rating11 from Guild_Appraisal_MASTER_tbl where emplid=" & Session("EMPLID_LOGON") & " and Perf_Year=" & Session("YEAR") & " )B,"
        ...
        SQL5 &= " (case when IndexID=10 then LTrim(Future_TaskDesc) else '' end)F_TaskDesc10 from Guild_FUTURE_FACTORS_TBL where emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & " )C )C"
        ...
        SQL6 &= " from Guild_FUTURE_FACTORS_TBL where Perf_year=" & Session("YEAR") & " and emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
    ...
    Protected Sub GLD_Submit_Click(ByVal sender As Object, ByVal e As EventArgs) Handles GLD_Submit.Click
        ...
        SQL = "Update Guild_Appraisal_MASTER_tbl Set Date_Guild_Reviewed ='" & Now & "',Guild_Comments='" & Replace(Trim(GuildComments.Text), "'", "`") & "' where emplid=" & Session("EMPLID_LOGON") & " and Perf_Year=" & Session("YEAR")
        SQL &= " Select Date_Guild_Reviewed,Guild_Comments from Guild_Appraisal_MASTER_tbl where emplid=" & Session("EMPLID_LOGON") & " and perf_year=" & Session("YEAR") & ""
        DT = LocalClass.ExecuteSQLDataSet(SQL)
        LocalClass.CloseSQLServerConnection()

        Session("Date_Guild_Reviewed") = DT.Rows(0)("Date_Guild_Reviewed").ToString
        Session("GuildComments") = Replace(DT.Rows(0)("Guild_Comments").ToString, Chr(13), "<br>")

        GLD_Submit.Visible = False : GuildComments.Visible = False : LblGuildComments.Visible = True
        LblGuild_Submitted.Text = "<font color=red><b>You have submitted your appraisal on " & Session("Date_Guild_Reviewed")
        LblGuildComments.Text = Session("GuildComments")


        Msg = Session("GUILD_NAME") & " has e-signed their October 1," & Session("Prev_Year").ToString & " - September 30," & Session("Cur_Year").ToString & " Appraisal with the following comments: <br><br>" & Session("GuildComments")
        Msg1 = "You e-signed October 1," & Session("Prev_Year").ToString & " - September 30," & Session("Cur_Year").ToString & " Appraisal with the following comments: <br><br>" & Session("GuildComments")
        'Msg1 &= "<br><br>Please click on the link http://" & Request.Url.Host & "/Guild_Performance_Appraisal/Default.aspx for full details."

        Msg2 = Session("GUILD_NAME") & " has e-signed their October 1," & Session("Prev_Year").ToString & "  - September 30," & Session("Cur_Year").ToString & " Appraisal. <br><br>"
        Msg3 = "You e-signed October 1," & Session("Prev_Year").ToString & " - September 30," & Session("Cur_Year").ToString & " Appraisal. <br><br>"
        'Msg3 &= "Please click on the link http://" & Request.Url.Host & "/Guild_Performance_Appraisal/Default.aspx for full details."

        If Len(Session("GuildComments")) > 0 Then
            'LocalClass.SendMail(Session("First_SUP_EMAIL"), "Guild Performance Appraisal comments was entered by " & Session("GUILD_NAME"), Msg)
            'LocalClass.SendMail(Session("HR_EMAIL"), "Guild Performance Appraisal comments was entered by " & Session("GUILD_NAME"), Msg)
            'LocalClass.SendMail(Session("GUILD_EMAIL"), "You have submmited your appraisal on " & Session("Date_Guild_Reviewed"), Msg)
        Else
            'LocalClass.SendMail(Session("First_SUP_EMAIL"), "Guild Performance Appraisal comments was entered by " & Session("GUILD_NAME"), Msg2)
            'LocalClass.SendMail(Session("GUILD_EMAIL"), "You have submitted your appraisal on " & Session("Date_Guild_Reviewed"), Msg3)
        End If
